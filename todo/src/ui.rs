...
pub fn ui_builder() -> impl Widget<TodoState> {
        ...
        .with_child(
            Button::new("->")
                ...
                if data.new_text.trim() != "" {
                    ...
                    data.todos.push_front(TodoItem { checked: false, text: text })
                }
            }))
        .with_child(Saver {});
        ...
        Flex::row()
            ...
            .with_child(Button::new("...").on_click(|ctx: &mut EventCtx, data: &mut TodoItem, _env| {
                ...
                let menu: Menu<TodoState> = Menu::empty()
                    .entry(MenuItem::new("Remove")
                    .on_activate(move |_, main_data: &mut TodoState, _| {
    ...
    ZStack::new(
        Flex::column()
            ...
            .with_aligned_child(Padding::new(5., clear_complete), UnitPoint::BOTTOM)
}
